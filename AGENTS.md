...
description: "DMF:GLOBAL, the principal behavioral contract for all projects under DMF"
date: "2026-03-10"
...
provenance:
  ...
  sources: ["PROJECT_STATEMENT_v1.0.0_CANON.md"]
---
...
# DMF Project Contract (this repository)

This file is the framework's own populated GLOBAL. It doubles as the
worked reference for what a filled-in contract looks like for one
principal. The bootstrap artifact lives at `template/AGENTS.md`; adopters
copy the template, run the INITIAL directives, and populate their own
LOCAL.
...
- **Workflow**: open an issue before substantive changes; fork, branch,
  update CHANGELOG.md, open a PR. See CONTRIBUTING.md.
- **Project-specific constraints**: the framework is markdown and JSON by
  design. No build, no install, no runtime. That is the constraint, and
  it is the feature.
